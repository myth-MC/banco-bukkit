...
public class ReloadSubcommand implements BiConsumer<CommandSender, String[]> {
    ...
    public void accept(CommandSender sender, String[] args) {
        MessageUtils.info(Banco.get().adventure().sender(sender), "banco.commands.banco.reload");
        Banco.get().reload();
        MessageUtils.success(Banco.get().adventure().sender(sender), "banco.commands.banco.reload.success");
    }
